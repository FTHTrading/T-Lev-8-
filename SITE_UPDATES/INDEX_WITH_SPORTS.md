...
## TROPTIONS Infrastructure Stack (Live)

1. **Token Launcher** — `launch.unykorn.org`
2. **Exchange OS** — `https://troptionslive.unykorn.org/exchange-os`
3. **Sports & Event Commerce** — `troptionslive.unykorn.org/sports` (DONK AI, fan NFTs, sponsor tiers, TTN, local commerce, Apostle/IPFS proof)
4. **Root NFT System** — `.troptions`, `.usa.26wc`, `.mexico.26wc`, `.canada.26wc`
...
0/8 cleared. No wire until all gates verified.
...
2. **Legacy domain redirects (Cloudflare/DNS):**
   ...
   - therealestateconnections.com → **park** until `aurora.unykorn.org` exists (interim: launch.unykorn.org)
   - green-n-go.solar → **park** until `impact.unykorn.org` exists
3. **Aurora demo mint:** https://launch.unykorn.org (not `aurora.unykorn.org` — NXDOMAIN)

See `REBRAND/LEGACY_DOMAIN_REDIRECTS.md` for verified targets.
